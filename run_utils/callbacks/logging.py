...
class LoggingEpochOutput(BaseCallbacks):
    ...
    def run(self, state, event):
        ...
        def get_serializable_values(output_format):
            ...
            for variable_type, variable_dict in output.items():
                for value_name, value in variable_dict.items():
                    code_list = [state.attached_engine_name, value_name]
        ...
        if state.global_state is not None:
            if not state.global_state.logging: 
                return
        else:
        ...
        # with open(state.log_info['log_file']) as fptr:
        #     prev_data = ruamel.yaml.load(fptr, Loader=ruamel.yaml.Loader) # a dict
        prev_data = joblib.load(state.log_info['log_file'])

        # ! fully integrate the update from simon collab back
        # tracker_code = '%s-%06d' % (run_affix, current_tracker)
        tracker_code = current_epoch
        ...
        # # TODO: may corrupt
        # with open(state.log_info['log_file'], "w") as fptr:
        #     ruamel.yaml.dump(prev_data, fptr)
